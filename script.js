const ageCalculate = () => {
    ...
    const inputDate = new Date(document.getElementById("input_date").value);
    const birthDetails = {
    ...
    if (isFutureDate(birthDetails, currentYear, currentMonth, currentDate)) {
        alert("Are You Sure That You Born ? ");
        displayResults("-", "-", "-");
        return;
    }
...
const calculateAge = (birthDetails, currentYear, currentMonth, currentDate) => {
    ...
    if (currentDate < birthDetails.date) {
        ...
        const daysInLastMonths = getDaysInMonth(lastMonth, currentYear);
        days = daysInLastMonths - (birthDetails.date - currentDate);
    } else {
...
document.getElementById("input_button").addEventListener("click", ageCalculate);
